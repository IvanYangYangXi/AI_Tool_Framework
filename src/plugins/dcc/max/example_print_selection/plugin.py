...
# 插件元数据常量
PLUGIN_NAME = "[示例] Max打印选择"
PLUGIN_VERSION = "1.0.0"
PLUGIN_TYPE = "dcc"
PLUGIN_DESCRIPTION = "获取3ds Max中选择的对象并打印其名称 - 示例脚本"
...
class PrintSelection:
    ...
    def connect_to_dcc(self) -> bool:
        ...
        try:
            ...
            logger.warning("3ds Max环境未找到，使用模拟模式")
            return False
    ...
    def get_selection(self) -> List[str]:
        ...
        try:
            if self._connected:
                from pymxs import runtime as rt
                selection = [obj.name for obj in rt.selection]
                return selection
            else:
                # 模拟模式
                logger.info("模拟模式：返回测试数据")
                return ["Box001", "Sphere001", "Cylinder001"]
